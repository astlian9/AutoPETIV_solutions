...
from utils import save_click_heatmaps, save_heatmaps
...
class Autopet_baseline:
    ...
    def load_inputs(self):
        ...
        if click_file:
            with open(os.path.join(self.lesion_click_path, click_file), 'r') as f:
                clicks = json.load(f)
            save_heatmaps(clicks, self.nii_path, 
                                os.path.join(self.nii_path, "TCIA_001_0001.nii.gz"),
                                )
    ...
    def predict(self):
        ...
        cproc = subprocess.run(
            f"nnUNetv2_predict -i {self.nii_path} -o {self.result_path} -d 140 -c 3d_fullres_resenc_bs240 -f 0 --disable_tta",
            shell=True,
            check=True,
        )
        print(cproc)
